                                            ...
                                            $datos1 = $ver[0]."||".
                                                ...
                                                <td>
                                                    <button class="btn btn-secondary bi bi-journal-plus"  data-bs-toggle="modal" data-bs-target="#modalAsignarElemento"  onclick="asignarElemento('<?php echo $datos1; ?>')"></button>
                                                     <button class="btn btn-warning bi bi-journal-plus"  data-bs-toggle="modal" data-bs-target="#modalMantenimientos"  onclick="asignarDatosMantenimiento('<?php echo $datos1; ?>')"></button>
                                                    </td>
                                                </tr>
                                                <?php
                                                    }
...
<div class="modal fade" id="modalMantenimientos" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="">
    <div class="modal-dialog modal-sm">
        <div class="modal-content" id="caja">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">Registrar Mantenimiento</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
                <div class="modal-body">
                <label for="">id Elemento</label>
                <br>
                <input type="text" id="ide" class="form-control input-sm" disabled>
                <?php  
                $mtto =   "SELECT * FROM tipos_mantenimientos";
                $resultado_mtto = mysqli_query($conexion, $mtto);
                ?>
                <br>
                <select class="form-select" id="id_tipo_mtto" aria-label="Default select example" required>
                <option selected>Tipo de Mantenimiento:</option>
                    <?php while($row = mysqli_fetch_assoc($resultado_mtto)): ?>
                ...
                $resultado_tecnicos = mysqli_query($conexion, $tecnicos);
                ?>
                <br>
                <select class="form-select" id="id_tecnicos" aria-label="Default select example" required>
                <option selected>Tecnico Encargado:</option>
                    <?php while($row = mysqli_fetch_assoc($resultado_tecnicos)): ?>
                    <option value="<?php echo $row['id']; ?>"><?php echo $row['nombres']; ?></option>
                <?php endwhile; ?>
                </select>
                <br> 
                <?php  
                $ciclos =   "SELECT * FROM ciclos";
                $resultado_ciclos = mysqli_query($conexion, $ciclos);
                ?>
                <br>
                <select class="form-select" id="id_ciclo" aria-label="Default select example" required>
                <option selected>Selecione Ciclo Mantenimiento:</option>
                    <?php while($row = mysqli_fetch_assoc($resultado_ciclos)): ?>
                    <option value="<?php echo $row['id']; ?>"><?php echo $row['nombres']; ?></option>
                <?php endwhile; ?>
                </select>
                <br>
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="revision" rows="3" oninput="validateNoNewLines(this)"></textarea>
                </div>
                <br>
                <div class="mb-3">
                    <label for="exampleFormControlTextarea1" class="form-label">Actividades Ejecutadas</label>
                    <textarea class="form-control" id="actividades" rows="3" oninput="validateNoNewLines(this)"></textarea>
                </div>
                <br>
                <div class="mb-3">
